...
exports.create = (globalObject, constructorArgs = [], privateData = {}) => {
  const ctor = globalObject[utils.ctorRegistrySymbol].SomeInterface;
  const obj = Object.create(ctor.prototype);
  const impl = new Impl(constructorArgs, privateData);
  utils.registerWrapper(obj, impl, $interfaceDescriptor);
  impl[utils.wrapperSymbol] = obj;
  return obj;
};

const $interfaceDescriptor = utils.createInterfaceDescriptor();

exports.is = obj => utils.implForWrapperWithInterface(obj, $interfaceDescriptor) !== null;
```
...
- `externalInterfaces`: an array of interface names whose constructors are supplied separately; generate installers for their static partial members instead of replacement constructors (see below)
...
### Static partial interfaces for external constructors

Normally, webidl2js merges partial interfaces into the full interface definition supplied to the same transformer. Use `externalInterfaces` when another package or the host environment supplies the constructor and you only need to add static members. For example, jsdom can generate the File API additions to whatwg-url's `URL` constructor without generating or temporarily installing a second constructor.

```js
const transformer = new WebIDL2JS({
  implSuffix: "-impl",
  externalInterfaces: ["URL"]
});
transformer.addSource("idl", "impls");
await transformer.generate("wrappers");
```

The source directory can then contain `URL.webidl`:
...
Provide `Blob`'s IDL and implementation through the same transformer's sources. Implement the additions as static members of `exports.implementation` in `impls/URL-impl.js`, just as for an ordinary interface implementation. `[WebIDL2JSCallWithGlobal]` passes the installation's `globalObject` as the first implementation argument, followed by the converted IDL arguments.

The generated `wrappers/URL.js` exports only `install(globalObject, globalNames)`. Install the original constructor first, then the additions:

```js
const { URL } = require("whatwg-url/webidl2js-wrapper");
const URLPartial = require("./wrappers/URL.js");

URL.install(globalObject, ["Window"]);
URLPartial.install(globalObject, ["Window"]);
```

The partial installer defines properties directly on `globalObject.URL`, preserving its identity, prototype, existing instances, and constructor registry entry. It does not require the constructor to have been generated by webidl2js. Argument conversions still use the bindings generated alongside the partial. Install separately in each realm so generated methods use that realm for exceptions and `[WebIDL2JSCallWithGlobal]`.

Static operations (including overloads within one partial) and static attributes (including readonly attributes) are supported. Each partial must explicitly specify `[Exposed]` with an identifier or identifier list, since the base interface's exposure is unknown. Multiple partials for the same interface produce one output module, with each partial retaining its own exposure and using the implementation directory of its source. The caller must ensure these exposure sets are compatible with the original interface. An installer does nothing when none of its partials are exposed in `globalNames`.

The following cases throw during generation, even with `suppressErrors: true`:

- Instance members, constants, stringifiers, iterables, and other non-static members.
- A full definition for a name listed in `externalInterfaces`, inheritance from such an interface, or an `includes` statement targeting it.
- Using an external interface as an IDL type, including in arguments, return types, attributes, dictionaries, or typedefs. This feature installs members; it does not import the external interface's type conversions.
- Duplicate member names, except for operation overloads within a single partial. Overloading across partial definitions is forbidden by Web IDL.
- Extended attributes on a partial other than `[Exposed]`, or on its members other than `[CEReactions]` and, for operations, `[WebIDL2JSCallWithGlobal]`. In particular, member-level `[Exposed]`, `[SecureContext]`, and `[SameObject]` are not supported here. Use separate partials for different exposure sets. Type and argument conversions otherwise have the same support as ordinary interfaces.

When an exposed partial is installed, it throws a `TypeError` if the target is missing or is not a function, or if any of its members would overwrite an existing own property of the constructor. Install each partial module once per realm, after the base constructor. The generated module has no `create()`, `setup()`, `convert()`, or brand-checking exports.
...
The object must not have been set up before: registering its private implementation and interface brand a second time throws a `TypeError`.
...
The generated wrapper files use modern JavaScript features such as private fields, private brand checks, `class` definitions, and `Proxy`. They require a JavaScript runtime with support for those features; webidl2js itself supports the Node.js versions listed in `package.json`.
...
Returns the corresponding wrapper class instance for a given implementation class instance, or `null` if the value has no associated wrapper.

### `tryWrapperForImpl(value)`

Returns the corresponding wrapper class instance for a given implementation class instance, or returns the argument back if it has no associated wrapper.
...
Returns the corresponding impl class instance for a given wrapper class instance, or `null` if the value has no associated implementation.
...
Returns the corresponding impl class instance for a given wrapper class instance, or returns the argument back if it has no associated implementation.
