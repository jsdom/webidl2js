...
Both hooks have the signature `(idl, code) => replacementCode`, where:

- `idl` is the parsed construct being operated on: an [operation member](https://github.com/w3c/webidl2.js#operation-member) or [attribute member](https://github.com/w3c/webidl2.js#attribute-member) for `[CEReactions]`, and an [interface](https://github.com/w3c/webidl2.js#interface) for `[HTMLConstructor]`;
...
If either hook is omitted, then the code will not be replaced, i.e. the default is equivalent to `(_, code) => code`.
