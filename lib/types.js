...
function generateTypeConversion(ctx, name, idlType, argAttrs) {
  ...
  if (typeof ${name} !== "object") {
    ...
    const conv = generateTypeConversion(ctx, "nextItem", idlType.idlType, []);
  ...
  if (typeof ${name} !== "object") {
    ...
    str += generateTypeConversion(ctx, "typedKey", idlType.idlType[0], []).body;

    const conv = generateTypeConversion(ctx, "typedValue", idlType.idlType[1], []);
    Object.assign(requires, conv.requires);
    str += conv.body;
