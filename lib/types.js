...
function generateTypeConversion(ctx, name, idlType, argAttrs = [], parentName, errPrefix = '"The provided value"') {
  ...
  const idlTypeRaw = idlType.idlType;
  if (idlType.union) {
    // union type
    generateUnion();
    ...
    // frozen array type
    generateFrozenArray();
  } else if (idlTypeRaw !== "any" && conversions[idlTypeRaw]) {
    // string or number type compatible with webidl-conversions
    generateGeneric(`conversions["${idlTypeRaw}"]`, idlTypeRaw === "object");
  } else if (resolvedTypes.has(ctx.typeOf(idlTypeRaw))) {
    // dictionaries, enumerations, and interfaces
    let fn;
    // Avoid requiring the interface itself
    if (idlTypeRaw !== parentName) {
      fn = `convert${idlTypeRaw}`;
      requires.add(idlTypeRaw, "convert");
    } else {
  ...
  function generateGeneric(conversionFn, tryUnwrap = false) {
    ...
    const [toImplStart = "", toImplEnd = ""] = tryUnwrap ? ["utils.tryImplForWrapper(", ")"] : [];
    if (idlType.array) {
      str += `
        for (let i = 0; i < ${name}.length; ++i) {
          ${name}[i] = ${toImplStart}${conversionFn}(${name}[i], { ${optString} })${toImplEnd};
        }
      `;
    } else {
      str += `
        ${name} = ${toImplStart}${conversionFn}(${name}, { ${optString} })${toImplEnd};
      `;
    }
  }
}
