...
function generateTypeConversion(
  ...
  if (idlType.union) {
    ...
    generateGeneric(`conversions["${idlType.idlType}"]`, "globals: utils.conversionGlobalsFromCtorRegistry(ctorRegistry)");
  } else if (resolvedTypes.has(ctx.typeOf(idlType.idlType))) {
    ...
    generateGeneric(fn, "ctorRegistry");
  } else {
    // unknown
  ...
  function generateUnion() {
...

    {
      ...
      if (type) {
        ...
        code += `throw new ctorRegistry["%TypeError%"](${errPrefix} + " is not of any supported type.")`;
      }
      code += "}";
      output.push(code);
    }
  ...
  function generateSequence() {
    ...
    str += `
      if (!utils.isObject(${name})) {
        throw new ctorRegistry["%TypeError%"](${errPrefix} + " is not an iterable object.");
      } else {
        const V = [];
        const tmp = ${name};
  ...
  function generateRecord() {
    ...
    str += `
      if (!utils.isObject(${name})) {
        throw new ctorRegistry["%TypeError%"](${errPrefix} + " is not an object.");
      } else {
  ...
  function generatePromise() {
    str += `${name} = new ctorRegistry["%Promise%"](resolve => resolve(${name}));`;
  }
  ...
  function generateGeneric(conversionFn, globalsParam) {
    ...
    let optString = `context: ${errPrefix}, ${globalsParam},`;
    if (clamp) {
