...
function resolveType(ctx, idlType, stack = []) {
  const conversions = utils.getVersionedConversions(ctx);
...
function generateTypeConversion(ctx, name, idlType, argAttrs = [], parentName, errPrefix = '"The provided value"') {
  ...
  if (idlType.union) {
    ...
    // frozen array type
    generateFrozenArray();
  } else if (ctx.options.skipConvertAny && idlType.idlType === "any") {
    // nothing to do (no point generating `curArg = curArg;`...)
  ...
  function generateUnion() {
    ...
    if (ctx.options.urlTypes) {
      if (union.URL || union.object) {
        output.push(`if (${name} instanceof URL) {}`);
      }

      if (union.URLSearchParams || union.object) {
        output.push(`if (${name} instanceof URLSearchParams) {}`);
      }
    }

    for (const importPath in ctx.options.externalTypes) {
      for (const type of ctx.options.externalTypes[importPath]) {
        if (union[type] || union.object) {
          if (!requires.has(type)) requires.addRaw(type, `require("${importPath}").${type}`);
...
function extractUnionInfo(ctx, idlType, errPrefix) {
  ...
  for (const item of idlType.idlType) {
    ...
    } else if (ctx.options.urlTypes && item.idlType === "URL") {
      if (seen.object) {
        error("URL is not distinguishable with object type");
      }
      seen.URL = true;
    } else if (ctx.options.urlTypes && item.idlType === "URLSearchParams") {
      if (seen.object) {
        error("URLSearchParams is not distinguishable with object type");
      }
      seen.URLSearchParams = true;
...
function sameType(ctx, type1, type2) {
  ...
  return sameType(ctx, extracted1.sequenceLike, extracted2.sequenceLike) &&
         ...
         extracted1.URL === extracted2.URL &&
         extracted1.URLSearchParams === extracted2.URLSearchParams &&
