    ...
    // for (let numArgs = minArgs; numArgs <= maxArgs; numArgs++) {
      ...
      if (i === d) {
        ...
        if (ctx.options.urlTypes) {
          const urls = S.filter(o => isOrIncludes(ctx, o.typeList[d], t => t.idlType === "URL"));
          if (urls.length) {
            possibilities.push(`
              if (curArg instanceof URL) {
                ${continued(urls[0], i)}
              }
            `);
          }

          const urlSearchParams = S.filter(o => isOrIncludes(ctx, o.typeList[d], t => t.idlType === "URLSearchParams"));
          if (urlSearchParams.length) {
            possibilities.push(`
              if (curArg instanceof URLSearchParams) {
                ${continued(urlSearchParams[0], i)}
              }
            `);
          }
        }
