...
function generateVarConversion(name, conversion, argAttrs, customTypes) {
  const requires = {};
  let str = "";
...
module.exports.generateOverloadConversions = function (overloads, customTypes) {
  ...
  for (let i = 0; i < arguments.length${extraClause}; ++i) {
    ...
    for (let i = 0; i < typeConversions.length; ++i) {
      ...
      const conv = generateVarConversion(`args[${i}]`, typeConversions[i], maxConstructor.operation.arguments[i].extAttrs, customTypes);
      Object.assign(requires, conv.requires);
      str += conv.body;
    }
  }
