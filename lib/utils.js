...
function generateBrandCheck(interfaceName, context, { receiver = "this ?? globalObject", lenient = false } = {}) {
  const message = `'${context}' called on an object that is not a valid instance of ${interfaceName}.`;
  const failure = lenient ? "return;" : `throw new globalObject.TypeError(${JSON.stringify(message)});`;
  return `
...
class RequiresMap extends Map {
  ...
  add(name, func = "") {
    const baseKey = func ? `$import_${toKey(func)}` : toKey(name);
    ...
    // Member imports are merged later, so allocate aliases across the generation context.
    let key = baseKey;
    let suffix = 2;
    while (this.ctx.importNames.has(key) && this.ctx.importNames.get(key) !== req) {
      key = `${baseKey}_${suffix++}`;
    }
    this.ctx.importNames.set(key, req);
