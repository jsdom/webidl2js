...
class Operation {
  ...
  generate() {
    ...
    if (
      this.ctx.options.skipWrapReturn ||
      overloads.every(overload => conversions[overload.operation.idlType.idlType])
    ) {
      str += `
      ...
      switch (this.generic) {
        ...
        case "record": {
          str += `
            const res = ${callOn}.${implFunc}(${argsSpread});
            if (res) Object.keys(res).forEach(function(k) { res[k] = utils.tryWrapperForImpl(res[k]); });
            return res;
          `;
          break;
        }
