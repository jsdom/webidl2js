...
class Operation {
  ...
  generate() {
    const conversions = utils.getVersionedConversions(this.ctx);
      ...
      switch (this.generic) {
        case "Promise": {
          str += `
            ...
            return res != null ? res.then(utils.tryWrapperForImpl) : res;
          `;
          break;
        }
        ...
        case "sequence": {
          str += `
            const res = ${callOn}.${implFunc}(${argsSpread});
            return res != null ? res.map(utils.tryWrapperForImpl) : res;
          `;
          break;
        }
        ...
        case "record": {
          str += `
            const res = ${callOn}.${implFunc}(${argsSpread});
            if (res != null) Object.keys(res).forEach(k => (res[k] = utils.tryWrapperForImpl(res[k])));
            return res;
          `;
          break;
        }
