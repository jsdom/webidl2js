...
class Operation {
  ...
  generate() {
    ...
    str += `
      Object.defineProperty(${targetObj}, ${JSON.stringify(this.name)}, {
        writable: true,
        enumerable: true,
        configurable: true,
        value: function ${fnName}(${argNames.join(", ")}) {
    `;
    if (!this.static) {
      str += `
    ...
    if (overloads.every(overload => conversions[overload.operation.idlType.idlType])) {
      str += `return ${callOn}.${implFunc}(${argsSpread});`;
    } else {
      str += `return utils.tryWrapperForImpl(${callOn}.${implFunc}(${argsSpread}));`;
    }
    str += "}});";
