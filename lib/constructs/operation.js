...
class Operation {
  ...
  hasCallWithGlobal() {
    ...
    if (hasCallWithGlobal && !this.static && !this.namespace) {
      throw new Error(
        `[WebIDL2JSCallWithGlobal] is only valid for static or namespace operations: ` +
        `"${this.name}" on ${this.interface.name}`
      );
    }
  ...
  generate() {
    ...
    if (!this.static && !this.namespace) {
      str += utils.generateBrandCheck(this.interface.name, this.name);
    }

    const callOn = this.static || this.namespace ? "Impl.implementation" : "$impl";
