  ...
  generateLegacyProxyHandler() {
    ...
    if (this.supportsNamedProperties && !this.isGlobal) {
      ...
      if (unforgeable.size > 0) {
        needFallback = true;
        definePropertyTrap += `if (!${JSON.stringify([...unforgeable])}.includes(P)) {`;
      }
      if (!overrideBuiltins) {
        needFallback = true;
        definePropertyTrap += "if (!utils.hasOwn(target, P)) {";
      }
    ...
    const setter = this.namedSetter || this.indexedSetter;
    if (setter && utils.hasCeReactions(setter)) {
      definePropertyTrap = this.ctx.processCEReactions(setter, definePropertyTrap);
    }
    ...
    if (this.namedDeleter && utils.hasCeReactions(this.namedDeleter)) {
      deleteTrap = this.ctx.processCEReactions(this.namedDeleter, deleteTrap);
    }
