...
class Interface {
  ...
  generateConstructor() {
    ...
    if (overloads.length !== 0) {
      ...
      const hasHTMLConstructor = Boolean(utils.getExtAttr(this.idl.extAttrs, "HTMLConstructor"));
      if (hasHTMLConstructor) {
        this.str += `
      ...
      let passArgs = conversions.hasArgs ? ", args" : "";
      if (!conversions.hasArgs && hasHTMLConstructor) {
        passArgs = ", []";
      }
      const privateData = hasHTMLConstructor ? `, { newTarget: new.target }` : "";
      this.str += `
          iface.setup(this${passArgs}${privateData});
        }
      `;
    } else {
      this.str += `
