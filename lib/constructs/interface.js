...
class Interface {
  ...
  _analyzeMembers() {
    ...
    if (this.hasNamedPropertiesObject) {
      this.factory = true;
    }
  }
  ...
  get hasNamedPropertiesObject() {
    return utils.isGlobal(this.idl) && this.supportsNamedProperties;
  }
  ...
  * allMembers(seen = new Set([this.name]), root = this.name) {
  ...
  _namedPropertyVisible(P, O, supports = false) {
    ...
    } else if (this.hasNamedPropertiesObject) {
      conditions.push(`!utils.hasOwn(${O}, ${P})`);
      conditions.push(`
        (() => {
          let prototype = Object.getPrototypeOf(${O});
          while (prototype !== null) {
            if (prototype !== namedPropertiesObject && utils.hasOwn(${O}, ${P})) {
              return false;
            }
  ...
  generateNamedPropertiesObject() {
    ...
    this.str += `
      const namedPropertiesObjectHandlersPlaceholder = {};
      ...
      }), namedPropertiesObjectHandlersPlaceholder);
      const namedPropertiesObjectHandlers = {
    `;
    ...
    this.str += `
        set(target, P, V, receiver) {
          ...
          const ownDesc = this.getOwnPropertyDescriptor(target, P);
  ...
  generateIface() {
    ...
    if (this.hasNamedPropertiesObject) {
      this.str += `
        enableNamedPropertiesObject(globalObject) {
          defaultPrivateData.globalObject = globalObject;
          Object.assign(namedPropertiesObjectHandlers, namedPropertiesObjectHandlersPlaceholder);
        },
      `;
    }
