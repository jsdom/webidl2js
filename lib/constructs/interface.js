  ...
  * allMembers(seen = new Set([this.name]), root = this.name) {
  ...
  addConstructor() {
    ...
    if (overloads.length !== 0) {
      ...
      body = `
        ${conversions.body}
        return iface.setup(Object.create(${this.name}.prototype)${passArgs});
      `;
    } else {
      body = `
  ...
  get defaultWhence() {
    return utils.isGlobal(this.idl) ? "instance" : "prototype";
  }
  ...
  generateOffInstanceAfterClass() {
    ...
    for (const [name, { whence, type, descriptor }] of this._outputMethods) {
      ...
      if (descriptor.configurable === true &&
          ...
          (type === "accessor" || descriptor.writable === true)) {
        continue;
      }
      if (type === "accessor") {
        protoProps.set(utils.stringifyPropertyName(name), `{
    ...
    for (const [name, { type, descriptor }] of this._outputStaticMethods) {
      ...
      if (type === "accessor") {
        classProps.set(utils.stringifyPropertyName(name), `{
          configurable: ${descriptor.configurable},
          enumerable: ${descriptor.enumerable}
        }`);
      } else {
  ...
  generateOnInstance() {
    ...
    for (const [name, { whence, type, args, body, descriptor }] of this._outputMethods) {
      ...
      if (type === "accessor") {
        props.set(utils.stringifyPropertyName(name), `{
          configurable: ${descriptor.configurable},
          enumerable: ${descriptor.enumerable}
        }`);
      } else {
    ...
    if (methods.length > 0 && propStrs.length > 0) {
      this.str += `
