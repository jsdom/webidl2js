...
class Interface {
  ...
  get hasNamedPropertyFastPath() {
    return this.isLegacyPlatformObj && this.supportsNamedProperties &&
      !utils.getExtAttr(this.idl.extAttrs, "LegacyOverrideBuiltins");
  }
  ...
  generateInstallIteratorPrototype() {
    ...
    if (iterable.isAsync) {
      ...
      if (iterable.isPair) {
        this.str += `
                  return newObjectInRealm(globalObject, utils.iteratorResult(next.map(utils.tryWrapperForImpl), internal.kind));
        `;
      } else {
        this.str += `
  ...
  * allMembers(seen = new Set([this.name]), root = this.name) {
  ...
  generateBrandCheck(context, { receiver = "this ?? globalObject", lenient = false } = {}) {
    if (!lenient) {
      this._needsBrandCheck = true;
      return `const $impl = $requireImpl(${receiver}, globalObject, ${JSON.stringify(context)});`;
    }
    return `
      const $impl = utils.implForWrapperWithInterface(${receiver}, $interfaceDescriptor);
      if ($impl === null) {
        return;
      }
    `;
  ...
  generateExport() {
    const parentDescriptor = this.idl.inheritance === null ?
      "" :
      `() => ${this.idl.inheritance}.interfaceDescriptor`;
    this.str += `
      const $interfaceDescriptor = utils.createInterfaceDescriptor(${parentDescriptor});
      exports.interfaceDescriptor = $interfaceDescriptor;
    `;

    if (this._needsBrandCheck) {
      this.str += `
  ...
  generateLegacyProxyHandler() {
    ...
    const supportsPropertyIndex = (O, index, indexedValue) => {
      ...
      if (unsupportedValue) {
        ...
        const value = indexedValue || `${O}${func}(${index})`;
        return `${value} !== ${unsupportedValue}`;
      }
      return `${O}[utils.supportsPropertyIndex](${index})`;
    };
    ...
    const supportsPropertyName = (O, P, namedValue) => {
      ...
      if (unsupportedValue) {
        ...
        const value = namedValue || `${O}${func}(${P})`;
        return `${value} !== ${unsupportedValue}`;
      }
      return `${O}[utils.supportsPropertyName](${P})`;
    };
    ...
    function namedPropertyVisible(P, O, supports = false) {
      const conditions = [];
      if (!supports) {
        conditions.push(supportsPropertyName("impl", P));
      }
      if (overrideBuiltins) {
    ...
    const generatePropertyLookup = ({ indexedResult, namedResult }) => {
      ...
      if (this.supportsIndexedProperties) {
        ...
        if (utils.getExtAttr(this.indexedGetter.extAttrs, "WebIDL2JSValueAsUnsupported")) {
          lookup = `
            const indexedValue = impl${func}(index);
            if (${supportsPropertyIndex("impl", "index", "indexedValue")}) {
              ${indexedResult("indexedValue")}
            }
          `;
        } else {
          lookup = `
            if (${supportsPropertyIndex("impl", "index")}) {
              const indexedValue = impl${func}(index);
              ${indexedResult("indexedValue")}
            }
          `;
      ...
      if (this.supportsNamedProperties) {
        ...
        if (utils.getExtAttr(this.namedGetter.extAttrs, "WebIDL2JSValueAsUnsupported")) {
          lookup = `
            const namedValue = impl${func}(P);
            if (${supportsPropertyName("impl", "P", "namedValue")} &&
        ...
        if (this.hasNamedPropertyFastPath) {
          if (this.idl.inheritance) {
            conditions.push("!this._hasPrototypeProperty(target, P)");
          } else {
            // The identity check only inspects an ordinary target, and `hasOwn()` only inspects the
            // captured ordinary prototype. Never walk an unrecognized prototype or invoke an accessor.
            conditions.push(`!(this._interfacePrototype !== null &&
    ...
    const invokeIndexedSetter = (O, P, V) => {
      ...
      if (!this.indexedSetter.name) {
        invocation = `
          ...
          if (creating) {
            ${O}[utils.indexedSetNew](index, indexedValue);
          } else {
            ${O}[utils.indexedSetExisting](index, indexedValue);
          }
        `;
      } else {
        invocation = `
          ${O}.${this.indexedSetter.name}(index, indexedValue);
        `;
      }
    ...
    const invokeNamedSetter = (O, P, V) => {
      ...
      if (!this.namedSetter.name) {
        invocation = `
          ...
          if (creating) {
            ${O}[utils.namedSetNew](${P}, namedValue);
          } else {
            ${O}[utils.namedSetExisting](${P}, namedValue);
          }
        `;
      } else {
        invocation = `
          ${O}.${this.namedSetter.name}(${P}, namedValue);
        `;
      }
    ...
    const prototypeParameter = this.idl.inheritance ? "interfacePrototypes" : "interfacePrototype";
    this.str += `
      const proxyHandlerCache = new WeakMap();
      class ProxyHandler {
        constructor(globalObject${this.hasNamedPropertyFastPath ? `, ${prototypeParameter}` : ""}) {
          this._globalObject = globalObject;${this.hasNamedPropertyFastPath ? `\nthis._${prototypeParameter} = ${prototypeParameter};` : ""}
        }
    `;

    if (this.hasNamedPropertyFastPath && this.idl.inheritance) {
      this.str += `
    ...
    if (this.supportsIndexedProperties) {
      ...
      let condition;
      if (utils.getExtAttr(this.indexedGetter.extAttrs, "WebIDL2JSValueAsUnsupported")) {
        this.str += `const indexedValue = impl${func}(index);`;
        condition = supportsPropertyIndex("impl", "index", "indexedValue");
      } else {
        preamble = `const indexedValue = impl${func}(index);`;
        condition = supportsPropertyIndex("impl", "index");
      }
    ...
    if (this.supportsIndexedProperties) {
      ...
      if (hasIndexedSetter) {
        this.str += `
            ...
            ${invokeIndexedSetter("impl", "P", "desc.value")}
            return true;
        `;
      } else {
        this.str += `
    ...
    if (this.supportsNamedProperties && !this.isGlobal) {
      ...
      if (!hasNamedSetter) {
        needFallback = true;
        this.str += `
          const creating = !(${supportsPropertyName("impl", "P")});
          if (!creating) {
            return false;
          }
        `;
        ...
        this.str += `
          ...
          ${invokeNamedSetter("impl", "P", "desc.value")}
          return true;
        `;
      }
    ...
    if (this.supportsIndexedProperties) {
      this.str += `
          if (utils.isArrayIndexPropName(P)) {
            const index = P >>> 0;
            return !(${supportsPropertyIndex("impl", "index")});
          }
      `;
    }
    if (this.supportsNamedProperties && !this.isGlobal) {
  ...
  generateIface() {
    ...
    const setup = this.hasNamedPropertyFastPath ? "setup" : "exports.setup";
    ...
    if (this.isLegacyPlatformObj) {
      this.str += `
        function makeProxy(wrapper, impl, globalObject${this.hasNamedPropertyFastPath ? ", isOrdinary" : ""}) {
      `;
      if (this.hasNamedPropertyFastPath) {
        this.str += `
      ...
      setWrapperToProxy = `
        wrapper = makeProxy(wrapper, impl, globalObject${this.hasNamedPropertyFastPath ? ", isOrdinary" : ""});`;
    }

    this.str += `
      exports.create = (globalObject, constructorArgs, privateData) => {
        const wrapper = makeWrapper(globalObject);
        return ${setup}(wrapper, globalObject, constructorArgs, privateData${this.hasNamedPropertyFastPath ? ", true" : ""});
      };
    ...
    this.str += `
      ...
      ${this.hasNamedPropertyFastPath ? "const setup" : "exports.setup"} =
          (wrapper, globalObject, constructorArgs = [], privateData = {}${this.hasNamedPropertyFastPath ? ", isOrdinary = false" : ""}) => {
      ...
      ${this.hasNamedPropertyFastPath ? "exports.setup = setup;" : ""}
      ...
      exports.new = (globalObject, newTarget) => {
        ...
        ${this.hasNamedPropertyFastPath ? "wrapper = makeProxy(wrapper, impl, globalObject, true);" : setWrapperToProxy}
  ...
  addConstructor() {
    ...
    if (overloads.length !== 0) {
      ...
      const ordinaryWrapperArgs = this.hasNamedPropertyFastPath ? ", undefined, true" : "";

      body = `
        ${conversions.body}
        return ${this.hasNamedPropertyFastPath ? "setup" : "exports.setup"}(${utils.formatArgs(setupArgs)}${ordinaryWrapperArgs});
      `;
    } else {
      body = `
  ...
  generateInstall() {
    ...
    if (this.hasNamedPropertyFastPath) {
      const prototypeArgument = this.idl.inheritance ?
        `[${[...this.allInterfaces()].map(iface => `ctorRegistry["${iface}"].prototype`).join(", ")}]` :
        `${name}.prototype`;
      this.str += `
        ...
        proxyHandlerCache.set(globalObject, {
          ordinary: new ProxyHandler(globalObject, ${prototypeArgument}),
          other: new ProxyHandler(globalObject, ${this.idl.inheritance ? "[]" : "null"})
        });
      `;
    }
