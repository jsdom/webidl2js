...
Interface.prototype.generateConstructor = function () {
  ...
  if (overloads.length !== 0) {
    ...
    const conversions = Parameters.generateConversions(overloads, this.opts.customTypes);
    if (conversions.prefix) {
      this.str += conversions.prefix + "\n\n";
    }
...
Interface.prototype.generateOperations = function () {
  ...
  for (let i = 0; i < this.idl.members.length; ++i) {
    ...
    if (member !== null) {
      ...
      this.str = data.prefix + this.str + data.body;
    }
  }
};
