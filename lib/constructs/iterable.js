...
class Iterable {
  ...
  generateFunction(key, kind) {
    this.interface.addMethod(this.interface.defaultWhence, key, [], `
      ${utils.generateBrandCheck(this.interface.name, key, { receiver: "this" })}
      return exports.createDefaultIterator(globalObject, $impl, "${kind}");
    `);
  }
  ...
  generate() {
    ...
    if (this.isPair) {
      ...
      this.interface.addMethod(whence, "forEach", ["callback"], `
        ${utils.generateBrandCheck(this.interface.name, "forEach", { receiver: "this" })}
