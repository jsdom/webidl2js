...
class Transformer {
  ...
  * _readFiles(files) {
  ...
  _parse(outputDir, contents) {
    ...
    this.ctx.initialize();
    const { interfaces, interfaceMixins, callbackFunctions, dictionaries, enumerations, typedefs } = this.ctx;
  ...
  * _writeFiles(outputDir) {
    ...
    const { interfaces, callbackFunctions, dictionaries, enumerations } = this.ctx;
    ...
    for (const obj of [...callbackFunctions.values(), ...dictionaries.values()]) {
      let source = obj.toString();
