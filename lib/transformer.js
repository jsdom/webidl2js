...
class Transformer {
  ...
  *_writeFiles(outputDir) {
    ...
    const interfaceStub = yield fs.readFile(__dirname + "/output/interfaceStub.js");
    for (const key of interfaces.keys()) {
      yield fs.writeFile(path.join(outputDir, key + ".js"), interfaceStub);
    }
    for (const key of dictionaries.keys()) {
      yield fs.writeFile(path.join(outputDir, key+ ".js"), "");
    }
