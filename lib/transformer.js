...
class Transformer {
  constructor({ implSuffix, ...options } = {}) {
    this.ctx = new Context({ implSuffix, options });
    this.ctx._externalTypes = [];

    if (options.externalTypes) {
      for (const key in options.externalTypes) {
        for (const type of options.externalTypes[key]) {
          this.ctx._externalTypes.push(type);
        }
      }
    }
  ...
  * _writeFiles(outputDir) {
    ...
    const { interfaces, dictionaries, enumerations, options } = this.ctx;
    const allExports = [];
    ...
    for (const obj of interfaces.values()) {
      ...
      source = `
        ...
        const conversions = require("webidl-conversions${options.urlTypes ? '/lib/node-v10.js' : ''}");
